...
fit <- svm(revenue ~ . , data = myData[1:n.train, ])
...
write.csv(submit, "res/my_svm_benchmark_drop.csv", quote = F, row.names = F)
#1840852.74559

rf.fit <- train(revenue ~ ., data = myData[1:n.train, ])
...
write.csv(rf.submit,  file = "res/rf_sub_drop.csv", row.names = F, quote = F)
#1753115.19882
...
write.csv(lr.submit,  file = "res/lr_sub_drop.csv", row.names = F, quote = F)
#
